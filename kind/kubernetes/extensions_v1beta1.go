...
func (k ingress) Informer(resync time.Duration, options ...informers.SharedInformerOption) kind.Informer {
	...
	ing := factory.Extensions().V1beta1().Ingresses()
	return &ingressInformer{factory: factory, informer: ing.Informer(), lister: ing.Lister()}
}
func (i ingressInformer) Informer() cache.SharedIndexInformer { return i.informer }
func (i ingressInformer) HasSynced() bool                     { return i.informer.HasSynced() }
func (i ingressInformer) Get(namespace, name string) (metav1.Object, error) {
	return i.lister.Ingresses(namespace).Get(name)
}
